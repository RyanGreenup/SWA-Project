...
\end{minted}
\caption{\label{orgc9aca1c}Load the Packages for \textbf{\textbf{\emph{R}}}}
\end{listing}
...
tk <-    rtweet::create_token(
  ...
  set_renv        = FALSE
\end{minted}
\caption{\label{orgf70e35f}Import the twitter tokens (redacted)}
\end{listing}

and hence all tweets containing a mention of \emph{Ubisoft} can be returned and saved to disk as shown in listing \ref{org96eb581}:
...
\end{minted}
\caption{\label{org96eb581}Save the Tweets to the HDD as an \texttt{rdata} file}
\end{listing}

\subsection{8.2.2 Count of Followers and Friends}
\label{sec:orgee043e7}
In order to identify the number of users that are contained in the \emph{tweets} the
\texttt{unique()} function can be used to return a vector of names which can then be passed as an index to the vector of counts as shown in listing \ref{org8a46764}, this provides that 81.7\% of the tweets are by unique users.
...
## > [1] 817
\end{minted}
\caption{\label{org8a46764}Return follower count of twitter posts}
\end{listing}


\subsection{8.1.3 Summary Statistics}
\label{sec:org55179f0}
The average number of friends and followers from users who posted tweets mentioning \emph{Ubisoft} can be returned using the \texttt{mean()} as shown in listing \ref{orgfb8c0be}
...
## > [1] 435.9449
\end{minted}
\caption{\label{orgfb8c0be}Determine the average number of friends and followers}
\end{listing}

\subsection{8.1.4 Above Average Followers}
\label{sec:org893ae9c}
...
will return the proportion of \texttt{TRUE} responses as shown in listing \ref{orgf9de3b0}. This
provides that:
...
## > [1] 0.2729498
\end{minted}
\caption{\label{orgf9de3b0}Calculate the proportion of users with above average follower counts}
\end{listing}


\subsection{8.1.5 Bootstrap confidence intervals}
\label{sec:org812625f}
\subsubsection{a/b.) Generate a bootsrap distribution}
\label{sec:orgd40e64a}
...
in listings \ref{orgd74843b} and \ref{org1e0cf70} and shown in figure \ref{fig:org8837024}.
...
\end{minted}
\caption{\label{orgd74843b}Bootstrapping a population from the sample.}
\end{listing}
...
\caption{\label{fig:org8837024}Histogram of the bootrapped population of follower counts}
\end{figure}

\subsubsection{c.) Estimate a Confidence Interval for the population mean Follower Counts}
\label{sec:orga51f8fb}
...
This is equivalent to drawing a sample from a population that is infinitely large and constructed of repetitions of the sample. This can be performed in \textbf{\emph{R}} as shown in listing \ref{orgea91021}.
...
##   588.4189 10228.7352
\end{minted}
\caption{\label{orgea91021}Confidence Interval of Mean Follower Count in Population}
\end{listing}
...
Such methods can be implemented in \textbf{\emph{R}} by passing a function to the the \texttt{boot} call as shown in listing \ref{org32155fe}. This provides a broader interval, providing that the true confidence interval could lie between 1079 and 16227 followers.
...
\end{minted}
\caption{\label{org32155fe}Bootstrap of population mean follower count implementing the \(BC_{a}\) method}
\end{listing}
...
below as shown in listing \ref{org091fe85}.
...
## Calculations and Intervals on Original Scale
\end{minted}
\caption{\label{org091fe85}Bootstrap of Proportion of Friends above average}
\end{listing}
\subsection{8.1.7 Is the Number of Friends Independent to the Number of Followers}
\label{sec:org3535c9c}
...
The counts may be binned by performing a logical interval test as shown in listing \ref{org43de4b7}.
...
\end{minted}
\caption{\label{org43de4b7}Use Logical Test to Assign observations into bins}
\end{listing}

\subsubsection{Find the Group frequency}
\label{sec:org162cd5a}
These values may be tabluated in order to count the occurence of users among these categories as shown in listing \ref{org25acf6e} and table \ref{tab:orgbde6420}.
...
## followers.y               2
\end{minted}
\caption{\label{org25acf6e}Tabulate the binned counts for the distribution of users among among amount and status.}
\end{listing}

\begin{table}[htbp]
\caption{\label{tab:orgbde6420}Table of Binned Friend and Follower counts, transposed relative to code.}
\centering
\begin{tabular}{lrr}
...
evaluated as shown in listing \ref{org2ee7fc2}.
...
chi_obs <- sum((e-o)^2/e)
\end{minted}
\caption{\label{org2ee7fc2}Calculate Expected frequency of values under the assumption of independence.}
\end{listing}

\begin{enumerate}
\item Testing Independence
\label{sec:orge878cf8}
...
shown in listing \ref{org9b0be01}, this provides a \(p\) -value < 0.0005, which means that the hypothesis of independence may be rejected with a high degree of certainty.
...
\end{minted}
\caption{\label{org9b0be01}Chi-Square testing for independence between friend and follower bin categories.}
\end{listing}

\begin{enumerate}
\item From First Principles
\label{sec:org4ffd5eb}
...
the \(p\) -value. This technique is demonstrated in listing \ref{orgca8430f}, the p-value
...
## [1] 4e-04
\end{minted}
\caption{\label{orgca8430f}Performing a \(\chi^{2}\) statistic from first principles}
\end{listing}
\end{enumerate}
\end{enumerate}
\subsubsection{{\bfseries\sffamily FIXME} Conclusion}
\label{sec:orgb7198ac}
...
This can be acheived easily using the \texttt{dplyr} package as shown in \ref{org0e0007b}, the top 20 of these users are shown in table \ref{tab:orga91025a} of the appendix
...
\end{minted}
\caption{\label{org0e0007b}Use \texttt{dplyr} to Filter for Users with a high Friend Count}
\end{listing}

\subsection{8.2.9 Find Users with Below Average Friend Counts}
\label{sec:org7582360}
Users with high friends may be determined by a similar method (or by taking the complement of the high friends) as shown in listing \ref{org1245ad4}, the lowest 20 of these users are shown in table \ref{tab:orgeefea2e} of the appendix.
...
\end{minted}
\caption{\label{org1245ad4}Use \texttt{dplyr} to Filter for Users with a low Friend Count}
\end{listing}

\subsection{8.2.10 Find the \emph{Tweets} corresponding to users with high or low friend counts}
\label{sec:org0d98676}
...
\end{minted}
\caption{\label{orga0516c4}Identify tweets corresponding to users with high and low friend counts}
\end{listing}

\subsection{8.2.11 Clean the tweets}
\label{sec:orga69877a}
\subsubsection{Create a Corpus Object}
\label{sec:orgbacf813}
In order to clean the tweets it is necessary to create a corpus object as shown in listing \ref{org28e15ee}, it is possible to pass a dataframe source in order to include the user ID, this isn't strictly necessary however because the \texttt{tm} package preserves order when performing transformations.
...
\end{minted}
\caption{\label{org28e15ee}Create a Corpus from the tweets}
\end{listing}
...
In order to encode the data as \emph{UTF-8}, the \texttt{iconv} function can be used as shown in listing \ref{org6173f55}.
...
\end{minted}
\caption{\label{org6173f55}Encode the Data as UTF-8}
\end{listing}

\subsubsection{Process the tweets}
\label{sec:org185cc6a}
...
The cleaning can be implemented by mapping functions over the corpus, which is fundamentally a list, this can be performed via the \texttt{tm\_map} function as shown in listing \ref{org0f1245d}.
...
\end{minted}
\caption{\label{org0f1245d}Use the \texttt{tm\_map} function to clean the tweets}
\end{listing}

\subsection{8.2.12 Display the first two tweets before/after processing}
\label{sec:org6a0c04c}
...
xkcd in figure \ref{fig:org6679179}.} as demonstrated in listing \ref{orgd73a809}, the first \emph{tweet} was rendered empty by the processing and the following two tweets were:
...
\end{minted}
\caption{\label{org160ec22}Load the Packages for \textbf{\textbf{\emph{R}}}}
\end{listing}

\item Apply TF-IDF Weighting
\label{sec:org08afb94}
...
This would requre multiplying each term of each row of the \(\mathrm{TF}_{t}\) matrix by the corresponding vector element of \(\mathrm{IDF}_{d,t}\), this can be implemented by taking the matrix product of a diagonalised matrix, this is shown in listing \ref{org9098be0}.
...
\end{minted}
\caption{\label{org9098be0}Apply TF-IDF Weigting}
\end{listing}
...
\label{sec:orgc3dc22e}
In order to create a term document matrix (and its transpose) with TF-IDF weighted values, the \texttt{weighting} argument may be specified as \texttt{weightTfIdf} by passing an appropriate list to the \texttt{control} argument of the \texttt{TermDocumentMatrix}, as shown in listing \ref{org0edc42f}
...
tweet_weighted_dtm <- as.DocumentTermMatrix()  %>%
                        as.matrix()
\end{minted}
\caption{\label{org0edc42f}Create a Document Term Matrix by transforming a Term Document Matrix}
\end{listing}

\subsubsection{Remove Empty Documents}
\label{sec:orge96f9d1}
Empty Documents may be removed from the matrix by a logical test as shown in listing  \ref{org34fca74} \footnote{It is important not to filter based the logic of an empty vector, because
otherwise an empty vector will returned, hence the if statement in listing \ref{org34fca74}.} this provides that 328 documents were empty following the processing. A summary of the first rows and columns of this matrix, following the removal of empty documents, is provided in table \ref{tab:orgdc9bfac} of the appendix.
...
\end{minted}
\caption{\label{org84ff311}Load the Packages for \textbf{\textbf{\emph{R}}}}
\end{listing}

\subsubsection{Measure Within Cluster Variance}
\label{sec:org97f452e}
...
implemented in listing \ref{orgdc645fe} and shown in figure \ref{fig:org8146977}.
...
\end{minted}
\caption{\label{orgdc645fe}Use a loop to evaluate the performace of various cluster models, plot this with \emph{ggplot2}}
\end{listing}

Figure \ref{fig:org8146977} Indicates a sudden stop of decrease in variance at 7 clusters and following that the within cluster variance begins to decrease at a slightly slower rate. For this reason 7 could be an appropriate candidate for the number of clusters, however the minimal amount of change in the within-cluster variance indicates that the data is most likely not clustered at all.


\begin{figure}[htbp]
\centering
\includegraphics[width=10cm]{./Figures/Q14WithinClusterVariance.png}
\caption{\label{fig:org8146977}Plot of the Within Cluster Variance of the tweets using cosine distance (projected into Euclidean Space)}
\end{figure}

\subsection{8.2.15 Find the Number of tweets in each cluster}
\label{sec:org81b9076}
Moving forward we'll use 3 clusters, 7 is too large and a smaller number will likely be more effective at categorising the data (particularly given that the stratification of the data appears to be quite limited from figure \ref{fig:org8146977}). The number of tweets in a cluster may be measured by using the \texttt{table} function as shown in listing \ref{org96775da} and table \ref{tab:org9e87b6b}.
...
table(K$cluster)
\end{minted}
\caption{\label{org96775da}The \texttt{table} function can count the number of tweets per cluster.}
\end{listing}

\begin{table}[htbp]
\caption{\label{tab:org9e87b6b}Number of tweets in each cluster identified by \(k\) means clustering}
\centering
\begin{tabular}{lrrr}
Cluster & 1 & 2 & 3\\
\hline
...
\ref{orge66b90a}. A plot of the friend countstaus mapped to shape and clusters mapped to
colour is demonstrated in listing \ref{orgef8dfd2} shown in \ref{fig:orga00fc7f}, In order to get a better
understanding of the distribution of friend counts a plot of the friend count
status mapped to colour is demonstrated in listing \ref{org6dc72b5} and shown in figure \ref{fig:orga00fc7f}.
...
\end{minted}
\caption{\label{orge66b90a}Use \emph{Multi-Dimensional} scaling to project the data into 2 dimensions}
\end{listing}
...
\end{minted}
\caption{\label{org6dc72b5}Create a plot of the distribution of Friends among Clusters as shown in figure \ref{fig:orgf3b4e3e}}
\end{listing}
...
\end{minted}
\caption{\label{orgef8dfd2}Create a plot of the distribution of Friends Count Status as shown in figure \ref{fig:orga00fc7f}}
\end{listing}

\begin{figure}[htbp]
\centering
\includegraphics[width=9cm]{./Figures/betclustplot8216.png}
\caption{\label{fig:orgf3b4e3e}Distribution of Friend Count Status over Clusters}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=9cm]{./Figures/betfriendplot8216.png}
\caption{\label{fig:orga00fc7f}Distribution of Friend Count Status}
\end{figure}

\subsection{8.2.17 Comment on the Visualisation}
\label{sec:orge748ba5}
Figure \ref{fig:orgf3b4e3e} indicates that altough there isn't a clear distinction between
clusters there is seperation of the tweets in such a way that does allow some
degree of classification to occur. Figure \ref{fig:orgf3b4e3e} and \ref{fig:orga00fc7f} both indicate that having
...
\end{minted}
\caption{\label{org1db8c28}Tabulate the distribution of friends in}
\end{listing}

\begin{table}[htbp]
\caption{\label{tab:org157e2f4}Above Average Friend Counts Across Clusters}
\centering
\begin{tabular}{lrrr}
...
\label{sec:org9dcd5b8}
Tweets can be sampled from the clusters by using the \texttt{sample} function with a logical test, this can be combined with a \texttt{for} loop as shown in listing \ref{org0a123e9}, this provides the following output.
...
\end{minted}
\caption{\label{org0a123e9}Sample Tweets from the Individual Clusters}
\end{listing}

\subsection{8.2.19 Identify themes in the Tweets}
\label{sec:org7f99ff6}
...
matrix based on a corpus cleaned without word stemming as shown in listing \ref{org5fa03af}.
Following that wordclouds can then be generated by indexing for the cluster,
this is demonstrated in listing \ref{orgdddaf0c} and shown in figures \ref{fig:orgfceeaed}, \ref{fig:org10dec0b} and \ref{fig:org3586d94}.
...
\end{minted}
\caption{\label{org5fa03af}Apply \emph{TF-IDF} weighting to an unstemmed corpus and then use a \texttt{for} loop to create wordclouds corresponding to each cluster.}
\end{listing}
...
\end{minted}
\caption{\label{orgdddaf0c}Apply \emph{TF-IDF} weighting to an unstemmed corpus and then use a \texttt{for} loop to create wordclouds corresponding to each cluster.}
\end{listing}

\begin{figure}[htbp]
\centering
\includegraphics[width=8cm]{./Figures/Cluster1Cloud.png}
\caption{\label{fig:orgfceeaed}Wordcloud of Cluster =\#=1 using \emph{TF-IDF} weighting}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=14cm]{./Figures/Cluster2Cloud.png}
\caption{\label{fig:org10dec0b}Wordcloud of Cluster =\#=2 using \emph{TF-IDF} weighting}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=10cm]{./Figures/Cluster3Cloud.png}
\caption{\label{fig:org3586d94}Wordcloud of Cluster =\#=3 using \emph{TF-IDF} weighting}
\end{figure}


\section{Appendix}
\label{sec:org1cfa227}
\subsection{Users with High Friend Count}
\label{sec:orge3adb16}

\begin{table}[htbp]
\caption{\label{tab:orga91025a}User ID and Friend Count of users with above highest friend count in sample}
\centering
\begin{tabular}{rr}
...
\label{sec:orgbfecc84}
\begin{table}[htbp]
\caption{\label{tab:orgeefea2e}User ID and Friend Count of users with above highest friend count in sample}
\centering
\begin{tabular}{rr}
\textbf{\emph{User ID}} & \textbf{\emph{Friend Count}}\\
\hline
...
\label{sec:orgc088d56}

\begin{table}[htbp]
\caption{\label{tab:orgdc9bfac}Document Term Matrix of first 6 documents and first 6 Words}
\centering
...
\label{sec:orgd580d22}
\begin{figure}[htbp]
\centering
\includegraphics[width=7cm]{./Figures/lisp_cycles.png}
\caption{\label{fig:org6679179}\emph{xkcd} \texttt{\#} 297}
\end{figure}
\end{document}
