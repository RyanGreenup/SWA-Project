...
\end{minted}
\caption{\label{org18c70b3}Load the Packages for \textbf{\textbf{\emph{R}}}}
\end{listing}
...
tk <-    rtweet::create_token(
  ...
  set_renv        = FALSE
\end{minted}
\caption{\label{orge37fed6}Import the twitter tokens (redacted)}
\end{listing}

and hence all tweets containing a mention of \emph{Ubisoft} can be returned and saved to disk as shown in listing \ref{org8214068}:
...
\end{minted}
\caption{\label{org8214068}Save the Tweets to the HDD as an \texttt{rdata} file}
\end{listing}

\subsection{8.2.2 Count of Followers and Friends}
\label{sec:org4cd44c7}
In order to identify the number of users that are contained in the \emph{tweets} the
\texttt{unique()} function can be used to return a vector of names which can then be passed as an index to the vector of counts as shown in listing \ref{org5348e8a}, this provides that 81.7\% of the tweets are by unique users.
...
## > [1] 817
\end{minted}
\caption{\label{org5348e8a}Return follower count of twitter posts}
\end{listing}


\subsection{8.1.3 Summary Statistics}
\label{sec:orgb49433d}
The average number of friends and followers from users who posted tweets mentioning \emph{Ubisoft} can be returned using the \texttt{mean()} as shown in listing \ref{orgb993b6b}
...
## > [1] 435.9449
\end{minted}
\caption{\label{orgb993b6b}Determine the average number of friends and followers}
\end{listing}

\subsection{8.1.4 Above Average Followers}
\label{sec:org50ae54f}
...
will return the proportion of \texttt{TRUE} responses as shown in listing \ref{org3e3b3b7}. This
provides that:
...
## > [1] 0.2729498
\end{minted}
\caption{\label{org3e3b3b7}Calculate the proportion of users with above average follower counts}
\end{listing}
...
in listings \ref{org7638036} and \ref{orgf1f434a} and shown in figure \ref{fig:org6d2f3e0}.
...
\end{minted}
\caption{\label{org7638036}Bootstrapping a population from the sample.}
\end{listing}
...
\caption{\label{fig:org6d2f3e0}Histogram of the bootrapped population of follower counts}
\end{figure}

\subsubsection{c.) Estimate a Confidence Interval for the population mean Follower Counts}
\label{sec:org9f23f92}
...
##   588.4189 10228.7352
\end{minted}
\caption{\label{org9b4e17a}Confidence Interval of Mean Follower Count in Population}
\end{listing}
...
Such methods can be implemented in \textbf{\emph{R}} by passing a function to the the \texttt{boot} call as shown in listing \ref{orge7fb5bb}. This provides a broader interval, providing that the true confidence interval could lie between 1079 and 16227 followers.
...
\end{minted}
\caption{\label{orge7fb5bb}Bootstrap of population mean follower count implementing the \(BC_{a}\) method}
\end{listing}
...
## Some BCa intervals may be unstable
\end{minted}
\caption{\label{org2df391f}Bootstrap of population mean follower count}
\end{listing}

\subsection{{\bfseries\sffamily FIXME} 8.1.6 Estimate a 97\% Confidence Interval for the High Friend Count Proportion}
\label{sec:orgefb7fb1}
In order to bootstrap a confidence interval for the proportion of users with
above average follower counts repeteadly draw random samples from an infinitely large population composed entirely of the sample, this can be acheived by resampling the observations of above and below as shown in listing \ref{org5e36f88}.
...
## 0.6976744 0.7601163
\end{minted}
\caption{\label{org5e36f88}Bootstrap of Proportion of Friends above average}
\end{listing}
\end{document}
