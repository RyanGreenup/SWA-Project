...
\end{minted}
\caption{\label{org46299db}Load the Packages for \textbf{\textbf{\emph{R}}}}
\end{listing}
...
tk <-    rtweet::create_token(
  ...
  set_renv        = FALSE
\end{minted}
\caption{\label{org06db666}Import the twitter tokens (redacted)}
\end{listing}

and hence all tweets containing a mention of \emph{Ubisoft} can be returned and saved to disk as shown in listing \ref{org83de683}:
...
\end{minted}
\caption{\label{org83de683}Save the Tweets to the HDD as an \texttt{rdata} file}
\end{listing}

\subsection{8.2.2 Count of Followers and Friends}
\label{sec:org1f3ee17}
In order to identify the number of users that are contained in the \emph{tweets} the
\texttt{unique()} function can be used to return a vector of names which can then be passed as an index to the vector of counts as shown in listing \ref{org7417d5b}, this provides that 81.7\% of the tweets are by unique users.
...
## > [1] 817
\end{minted}
\caption{\label{org7417d5b}Return follower count of twitter posts}
\end{listing}


\subsection{8.1.3 Summary Statistics}
\label{sec:org646b157}
The average number of friends and followers from users who posted tweets mentioning \emph{Ubisoft} can be returned using the \texttt{mean()} as shown in listing \ref{orgc4f0915}
...
## > [1] 435.9449
\end{minted}
\caption{\label{orgc4f0915}Determine the average number of friends and followers}
\end{listing}

\subsection{8.1.4 Above Average Followers}
\label{sec:org0aed482}
...
will return the proportion of \texttt{TRUE} responses as shown in listing \ref{orgf9f75cb}. This
...
## > [1] 0.2729498
\end{minted}
\caption{\label{orgf9f75cb}Calculate the proportion of users with above average follower counts}
\end{listing}


\subsection{8.1.5 Generate a bootsrap distribution of the Follower Counts}
\label{sec:org74cc531}
...
listing \ref{org8c6a857} and figure \ref{fig:org18db668}.
...
\end{minted}
\caption{\label{org8c6a857}Bootstrapping a population from the sample.}
\end{listing}
...
\caption{\label{fig:org18db668}Histogram of the bootrapped population of follower counts}
\end{figure}

\section{References}
\label{sec:org5b7ad37}
\label{org77541db}
