...
- It's worth noting that the normal $t$ value bootstrap offers no advantage over
 ...
 Such methods can be implemented in */R/* by passing a function to the =boot= call as shown in listing [[bootx]]. This provides a broader interval, providing that the true confidence interval could lie between 1079 and 16227 followers.
...
This can be achieved easily using the =dplyr= package as shown in [[hfdp]], the top 20 of these users are shown in table [[hfls]] of the appendix
...
In order to effectively visualise this graph, the number of nodes will be limited to 13, this can be achieved by filtering the list for more than 13 observations as shown in listing [[shm]].
