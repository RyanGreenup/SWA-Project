...
- It's worth noting that the normal $t$ value bootstrap offers no advantage over
  ...
  cite:ripley2020 in their texbook /Bootstrap Methods and their Application/.
...
This provides that:
 + The 97% confidence interval for the population proportion of users that have
  ...
  + i.e. The probability of any given sample containing the population mean
...
acheive this in **_R_** the identity from equation eqref:cos can be used and
...
#+NAME: dn1c
#+CAPTION: Create a dendrogram of the terms in the cluster with the higest friends count, average linkage was used.
#+begin_src R
...
The tweets can be downloaded by using the =get_friends= and =lookup_users=
...
In order to effectively visualise this graph, the number of nodes will be limited to 13, this can be acheived by filtering the list for more than 13 observations as shown in listing [[shm]].
...
*** Use GGPlot2
This Graph could also be produced using =ggplot2= and with an alternate algorithm for aranging the nodes, in order to do this a /tidy/ data frame needs to be produces as demonstrated in listing [[tdy]] and then the ggplot function can be used to build a plot as demonstrated in listing [[ggp]], this produces an alternative visualsation of the graph as shownin figure [[ggpt]].
...
4. GailSimone
  ...
  Unsuprisingly /Ubisoft/ is the most central person, this is expected because the graph was produced from the perspective of /Ubisoft/ and the nodes had a very limited degree of inteconnectivity.
...
/Ubisoft/ has many connections as shown in figures [[gp1]] and [[ggpt]], these connections however have a limited amount of interconnectivity.
  ...
  Suprisingly however, Gail Simone, a comic book author involved in works such as /Batgirl/ and /Birds of Prey/, is a very central character, in combination with the central connection to /Arbys/ this is indicative of the interests and lifestyle of the typical demographic of friends of the /Ubisoft/ account.
* Appendix
