...
[fn:2] The =[= function is actually shorthand for =Extract()=, most things in */R/* are functions, this is similar to /LISP/ and has to do with the origins of the language, e.g. ~sum(1:10) == (sum (1:10))~, also relevant see the relevant xkcd in figure [[xkcd]].
