...
relevant posts can be retrieved from twitter by leveraging the =rtweet= package, packages can be loaded for use in **_R_** thusly:
...
and all tweets containing a mention of /Ubisoft/ can be returned:
